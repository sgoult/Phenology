...
netcdf_file = "sst_reshaped_new.nc"

output = "sst_reshaped_new_5d_comp.nc"
...
for year in years:
    ...
    if len(input_file.sst.TIME.dt.year.isin([year])) != 12:
        ...
        sst = yr_sst.interp(TIME=pandas.date_range(start=datetime.datetime(year=year, month=min_month, day=1), end= datetime.datetime(year=year, month=max_month, day=31), freq='8D'))  
    else:
    ...
    sst.to_netcdf(path="preproc_interp_to_delete_{}.nc".format(year))

sst=None

joined_arrays = xarray.open_mfdataset('preproc_interp_to_delete_*.nc', concat_dim='TIME')
